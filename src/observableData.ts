...
const proxyBro = Symbol("_proxy_bro");
/**
    ...
    let creatProxyBro = (obj: object, notifyParent?: () => void, props?: string) => {
        ...
        let proxy = new Proxy(obj, {
            set: (target, props, value) => {
                ...
                let bro: { proxy: any, eventEmitter: EventEmitter } = Reflect.get(target, proxyBro);
            ...
            get: (target, props) => {
                ...
                if (props == proxyBro) return value;

                // console.log(`get ${props.toString()}`);
                let parentBro: { proxy: any, eventEmitter: EventEmitter } = Reflect.get(target, proxyBro);

                let bro: { proxy: any, eventEmitter: EventEmitter } = Reflect.get(value, proxyBro);
                if (bro == null && typeof value == "object") {
                    creatProxyBro(value, () => {
                        // console.log("notify-parent", props);
                        parentBro.eventEmitter.emit(props as any, value);
                    }, props as any);
                    bro = Reflect.get(value, proxyBro);
                }
                return value?.[proxyBro]?.proxy ?? value
            }
        });
        Reflect.set(obj, proxyBro, { proxy, eventEmitter: new EventEmitter() });
        return proxy;
    }
